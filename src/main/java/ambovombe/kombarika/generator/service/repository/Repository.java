...
public class Repository {
  ...
  public String getRepositoryImport(String packageName, String table) throws Exception {
    ...
    for (String item : this.getFrameworkProperties().getImports().getRepository())
      res +=
          imp
              + " "
              + item.replace("packageName", packageName)
                  .replace(
  ...
  public String getRepositoryClass(String table, List<String> primaryKeysType) throws Exception {
    ...
    res =
        res.replace(
            "#type#", this.getTypeMapping().getListMapping().get(primaryKeysType.get(0)).getType());
    return res;
  }

  public String generateRepository(
      String table, String packageName, String entityPackage, List<String> primaryKeysType)
      throws Exception {
    String res = "";
    if (this.getFrameworkProperties().getRepository().equals("")) return res;
    String path =
        Misc.getSourceTemplateLocation()
            .concat(File.separator)
            .concat(this.getFrameworkProperties().getRepository());
    ...
    for (String string : tables) {
      field +=
          this.getFrameworkProperties()
                  ...
                  .getFieldSyntax()
                  .replace(
                      "#Field#", ObjectUtility.capitalize(ObjectUtility.formatToCamelCase(string)))
              + "\n";
    }
    res =
        template
