...
public class Entity {
  ...
  public String getEntityClass(String table) {
    ...
    if (!this.getAnnotationProperty().getEntity().equals(""))
      res +=
          this.getLanguageProperties()
                  .getAnnotationSyntax()
                  .replace("?", this.getAnnotationProperty().getEntity())
              + "\n";
    res +=
        this.getLanguageProperties()
                .getAnnotationSyntax()
                .replace("?", this.getAnnotationProperty().getTable())
                .replace("?", table)
            + "\n"
  ...
  public Set<String> getAllImports(HashMap<String, String> columns) {
    ...
    for (Map.Entry<String, String> set : columns.entrySet()) {
      if (this.getTypeMapping().getListMapping().get(set.getValue()).getPackageName().equals("")) {
        continue;
      }
      lst.add(
          this.getTypeMapping().getListMapping().get(set.getValue()).getPackageName()
              + ""
      ...
      if (primaryKeys.contains(set.getKey())) {
        res +=
            "\t"
                + this.getLanguageProperties()
                    .getAnnotationSyntax()
                    .replace("?", this.getAnnotationProperty().getConstraints().getPrimaryKey())
                + "\n";
        if (set.getValue().equals("Integer")
            && !this.getAnnotationProperty().getAutoIncrement().equals("")) {
          res +=
              "\t"
                  + this.getLanguageProperties()
                      .getAnnotationSyntax()
                      .replace("?", this.getAnnotationProperty().getAutoIncrement())
                  + "\n";
        }
      }
        ...
        res +=
            "\t"
                + this.getLanguageProperties()
                    ...
                    .replace(
                        "?",
                        this.getAnnotationProperty()
                            .getConstraints()
                            .getForeignKey()
                            .getAnnotation()
                            .replace("?", set.getKey()))
                + "\n";
        if (!this.getAnnotationProperty()
            .getConstraints()
            .getForeignKey()
            .getManyToOne()
            .equals("")) {
        ...
        res +=
            "\t"
                + this.getLanguageProperties()
                    .getFieldSyntax()
                    .replace(
                        "Type", ObjectUtility.capitalize(ObjectUtility.formatToCamelCase(temp)))
  ...
  public String getEncapsulation(
      HashMap<String, String> columns, HashMap<String, String> foreignKeys) {
    String rep = "";
    for (Map.Entry<String, String> set : columns.entrySet()) {
      ...
      rep +=
          this.getLanguageProperties()
              ...
              .replace(
                  "#Field#",
                  ObjectUtility.formatToCamelCase(ObjectUtility.capitalize(set.getKey())))
              .replace("#field#", ObjectUtility.formatToCamelCase(set.getKey()));
    }
    return rep;
  }
  ...
  public String generateEntity(
      DbConnection dbConnection, String template, String table, String packageName)
      throws Exception {
    HashMap<String, String> columns =
    ...
    String res =
        template
            .replace(
                "#package#", GeneratorService.getPackage(this.getLanguageProperties(), packageName))
  ...
  public String getFileName(String table) {
    return ObjectUtility.capitalize(ObjectUtility.formatToCamelCase(table))
        .concat("." + this.getLanguageProperties().getExtension());
  }
